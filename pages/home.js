import { APPLICATION_STAGES, applicationStage, escapeHtml, formatDate, formatDateTime, statusClass, groupBy, button } from "../shared/utils.js";
...
function render() {
  ...
  const grouped = groupBy(state.applications, app => applicationStage(app.status));
  outlet.innerHTML = `
    ...
    <div class="v2-grid">
      ...
      <section class="v2-card span-12"><div class="v2-card-head"><div><h3>申请分流</h3><span class="small muted">完成测评仍留在对应阶段；只有 Offer / Rejected 才计入结果</span></div><a class="link-button" data-route="applications" href="${window.BBRouteUrl("applications")}">查看申请追踪 →</a></div><div class="v2-card-body"><div class="stage-strip">${APPLICATION_STAGES.map(stage => `<div class="stage"><h4>${stage}<span class="stage-count">${(grouped[stage] || []).length}</span></h4><div class="stage-items">${(grouped[stage] || []).slice(0, 4).map(app => `<span>${escapeHtml(app.company)} · ${escapeHtml(app.division || app.role)}</span>`).join("") || `<em class="muted small">暂无</em>`}</div></div>`).join("")}</div></div></section>
