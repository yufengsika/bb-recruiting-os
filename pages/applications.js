...
function render() {
  ...
  const rows = state.applications.filter(app => !filter || app.status === filter || app.company === filter);
...
export function mount(target) {
  outlet = target; render();
  onChange = event => { if (event.target.id === "application-filter") { filter = event.target.value; render(); } if (event.target.matches("[data-status-app]")) window.BBStore.dispatch({ type: "application.statusChanged", applicationId: event.target.dataset.statusApp, status: event.target.value }); };
  onClick = event => { if (event.target.id === "application-search") return; };
  onInput = event => { if (event.target.id === "application-search") { filter = event.target.value; render(); document.getElementById("application-search")?.focus(); } };
  outlet.addEventListener("change", onChange); outlet.addEventListener("input", onInput);
}
export function unmount() { outlet?.removeEventListener("change", onChange); outlet?.removeEventListener("input", onInput); outlet = null; onClick = null; onChange = null; onInput = null; }
