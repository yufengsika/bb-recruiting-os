import { APPLICATION_STAGES, applicationStage, escapeHtml, formatDate, statusClass, logoUrl } from "../shared/utils.js";
...
function render() {
  ...
  const groups = Object.fromEntries(APPLICATION_STAGES.map(stage => [stage, rows.filter(app => applicationStage(app.status) === stage)]));
  ...
  outlet.innerHTML = `<div class="page-intro"><div><h2>申请追踪</h2><p>${companies} 家公司，${state.applications.filter(app => !/wishlist|research/i.test(app.status)).length} 个独立 programme。每一行都是一个申请 ID。</p></div><div class="small muted">${state.applications.length} records</div></div>
    ...
    <section class="v2-card"><div class="v2-card-head"><div><h3>申请分流图</h3><span class="small muted">完成 OT / VI 仍属于对应阶段；只有 Offer / Rejected 才计入结果</span></div></div><div class="v2-card-body"><div class="stage-strip">${APPLICATION_STAGES.map(stage => `<div class="stage"><h4>${stage}<span class="stage-count">${groups[stage].length}</span></h4><div class="stage-items">${groups[stage].slice(0, 8).map(app => `<span>${escapeHtml(app.company)} · ${escapeHtml(app.division || app.role)}</span>`).join("") || `<em class="muted small">暂无</em>`}</div></div>`).join("")}</div></div></section>
